...
2. **Inicial + apellido**: el nickname tiene forma "X. Apellido" y worldcup26 tiene un nombre que empieza por esa misma letra y termina en ese apellido. Ejemplos: `T. Martínez` ↔ "Toni Martínez", `M. Aguado` ↔ "Marc Aguado", `E. Ponce` ↔ "Ezequiel Ponce". Va **antes** que la regla de apellido (4): es estrictamente más discriminante (mismo requisito de apellido, más la inicial), así que si corriera después, un compañero de equipo con el mismo apellido y nickname solo-apellido podría quedar atascado como ambiguo antes de que esta regla libere al candidato correcto (confirmado con un test: `A. García`/`García` contra `Andrés García`/`Kike García` solo resuelve los dos correctamente en este orden).
